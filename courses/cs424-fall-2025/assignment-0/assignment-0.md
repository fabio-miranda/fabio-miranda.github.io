...
No two groups can select the same dataset. Dataset selection will be handled on a first come, first served basis. Check if your preferred dataset has already been claimed; if so, you will need to select a different one.
...
- -25 for not correctly adding netids to the spreadsheet.
